...
module.exports = {
  ...
  create: async (req, res) => {
    try {
      ...
      const usuarioExistente = await usuariosModel.getByUsername(nombre_usuario);
      if (usuarioExistente) {
      ...
      const id_usuario = await usuariosModel.create({ nombre_usuario, pin: hashedPin, id_trabajador, id_rol });
  ...
  update: async (req, res) => {
    ...
    try {
      ...
      if (id_trabajador) updateData.id_trabajador = id_trabajador;
      ...
      if (filasAfectadas === 0) {
        return res.status(404).json({ error: 'Usuario no encontrado o no se hicieron cambios' });
      }

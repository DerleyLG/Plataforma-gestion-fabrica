...
getByOrdenes: async (ids_orden_fabricacion) => {
  ...
  create: async ({ id_orden_fabricacion, id_articulo, id_etapa_produccion, id_trabajador, cantidad, estado, observaciones = null, costo_fabricacion }, connection = db) => {
...
actualizarEstadoOrden: async (id_orden_fabricacion, nuevoEstado, connection = db) => {
  ...
  getCantidadTotalArticuloEnOrden: async (idOrden, idArticulo, connection = db) => {
    const query = `
      SELECT cantidad
  ...
  getCantidadRegistradaEnEtapa: async (idOrden, idArticulo, idEtapa, connection = db) => {
    const query = `
      SELECT SUM(cantidad) AS total
      FROM avance_etapas_produccion
      WHERE id_orden_fabricacion = ? AND id_articulo = ? AND id_etapa_produccion = ?
    `;
    const [rows] = await (connection || db).query(query, [idOrden, idArticulo, idEtapa]);
    return rows[0].total || 0;
  },

  getEtapasTotalmenteCompletadas: async (idOrdenFabricacion, idArticulo, connection = db) => {
    const [rows] = await (connection || db).query(`
      SELECT 
        a.id_etapa_produccion,
  ...
  getCantidadTotalArticulo: async (id_orden_fabricacion, id_articulo, connection = db) => {
    const [rows] = await (connection || db).query(
      `SELECT cantidad 
       FROM detalle_orden_fabricacion 
       WHERE id_orden_fabricacion = ? AND id_articulo = ?`,
  ...
  getCantidadRegistradaEtapa: async (id_orden_fabricacion, id_articulo, id_etapa_produccion, connection = db) => {
    const [rows] = await (connection || db).query(
      `SELECT SUM(cantidad) AS total 
       FROM avance_etapas_produccion 
  ...
  puedeRegistrarAvance: async (id_orden_fabricacion, id_articulo, id_etapa, cantidadSolicitada, connection = db) => {
...
const loteId = await LoteModel.createLote({
  ...
  actualizarEstadoAvancesEtapa: async (id_orden_fabricacion, id_articulo, id_etapa_produccion, connection = db) => {

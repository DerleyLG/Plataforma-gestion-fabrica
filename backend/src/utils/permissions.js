...
function can(role, action) {
  ...
  console.log(`[can] Rol: "${role}", Acción: "${action}", Permisos del rol:`, allowed, '-> Resultado:', hasPermission);
...
function requirePermission(action) {
  return (req, res, next) => {
    ...
    console.log(`[requirePermission] Verificando permiso "${action}" para rol "${req.user.rol}":`, hasPermission ? 'PERMITIDO' : 'DENEGADO');
    
    if (!hasPermission) {
      console.warn(`[requirePermission] Acceso denegado. Rol: ${req.user.rol}, Acción: ${action}`);
      return res
        .status(403)

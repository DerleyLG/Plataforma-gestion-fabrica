...
import { FiTrash2, FiPlus, FiArrowLeft, FiChevronDown, FiChevronRight } from 'react-icons/fi';
  ...
  const toggleExpand = async (idCosto, e) => {
    ...
    if (yaExpandido) {
      ...
      if (!asignacionesDetalle[idCosto]) {
        try {
          const res = await api.get(`/costos-indirectos-asignados/costo/${idCosto}`);
          const asignaciones = Array.isArray(res.data) ? res.data : [];
          setAsignacionesDetalle(prev => ({ ...prev, [idCosto]: asignaciones }));
        } catch (error) {
          ...
          <div className="inline-flex items-center gap-3">
            ...
            <div className="flex items-center rounded-md border border-slate-300 overflow-hidden h-[44px]" role="group" aria-label="Filtrar estado">
              <button
                ...
                className={`cursor-pointer px-3 h-full text-sm ${filterMode==='registrados' ? 'bg-slate-800 text-white' : 'bg-white text-slate-700'}`}
                title="Registrados"
              >Registrados</button>
              <button
                type="button"
                onClick={() => setFilterMode('asignados')}
                className={`cursor-pointer px-3 h-full text-sm border-l ${filterMode==='asignados' ? 'bg-slate-800 text-white' : 'bg-white text-slate-700'}`}
                title="Asignados"
              >Asignados</button>
              <button
                type="button"
                onClick={() => setFilterMode('todos')}
                className={  `cursor-pointer px-3 h-full text-sm border-l ${filterMode==='todos' ? 'bg-slate-800 text-white' : 'bg-white text-slate-700'}`}
                title="Todos"
              >Todos</button>
            </div>
          </div>
          ...
          <tbody>
            {filteredCostos.length > 0 ? (
              filteredCostos.map((costo) => {
                ...
                const detalles = asignacionesDetalle[costo.id_costo_indirecto] || [];
                
                return (
                  <React.Fragment key={costo.id_costo_indirecto}>
                    <tr
                      onClick={() => tieneAsignaciones && toggleExpand(costo.id_costo_indirecto)}
                      className={`transition select-none ${tieneAsignaciones ? 'hover:bg-slate-200 cursor-pointer' : 'hover:bg-slate-100'}`}
                    >
                      <td className="px-4 py-3">
                        {tieneAsignaciones && (
                          <button
                            onClick={(e) => toggleExpand(costo.id_costo_indirecto, e)}
                      ...
                      <td className="px-4 py-3">
                        {costo.fecha_inicio && costo.fecha_fin ? (
                          ...
                          costo.fecha ? new Date(costo.fecha.split('T')[0] + 'T00:00:00').toLocaleDateString('es-CO') : '-'
                        )}
                      </td>
                      <td className="px-4 py-3">
                        {costo.fecha_inicio && costo.fecha_fin ? (
                      ...
                      <td className="px-4 py-3">
                        {(() => {
                          const total = Number(costo.valor || 0);
                          if (!total || asignado <= 0) {
                            return <span className="px-2 py-1 text-xs rounded bg-gray-100 text-gray-800">Registrado</span>;
                          }
                          if (asignado >= total) {
                    ...
                    {expandido && tieneAsignaciones && (
                      <tr className="bg-slate-50">
                        <td colSpan="8" className="px-4 py-3">
                          <div className="ml-8">
                            <h4 className="text-sm font-semibold text-slate-700 mb-3">Asignaciones:</h4>
                            {detalles.length > 0 ? (
                              <div className="space-y-2">
                                {detalles.map((asig, idx) => {
                                  const nombreCompleto = asig.nombre_cliente && asig.apellido_cliente 
                                    ? `${asig.nombre_cliente} ${asig.apellido_cliente}` 
                                    : asig.nombre_cliente || 'Sin cliente';
                                  
                                  return (
                                    <div key={idx} className="border border-slate-200 rounded-md p-3 bg-white">
                                      <div className="flex items-start gap-4 flex-wrap">
                                        <div className="flex-shrink-0">
                                          <span className="font-bold text-slate-700">OF #{asig.id_orden_fabricacion}</span>
                                          <span className={`ml-2 px-2 py-0.5 text-xs rounded ${
                                            asig.estado === 'completada' ? 'bg-green-100 text-green-800' :
                                            asig.estado === 'en proceso' ? 'bg-blue-100 text-blue-800' :
                                            asig.estado === 'pendiente' ? 'bg-yellow-100 text-yellow-800' :
                                        ...
                                        <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                                          <div>
                                            ...
                                            <span className="ml-1 font-medium text-slate-700">{nombreCompleto}</span>
                                          </div>
                                          
                                          {asig.cantidad && (
                                            <div>
                                              <span className="text-slate-500">Cantidad:</span>
                                              <span className="ml-1 font-medium text-slate-700">{asig.cantidad}</span>
                                            </div>
                                          )}
                                          ...
                                          {asig.fecha_inicio && (
                                            <div>
                                              ...
                                              <span className="ml-1 font-medium text-slate-700">
                                                {new Date(asig.fecha_inicio.split('T')[0] + 'T00:00:00').toLocaleDateString('es-CO')}
                                              </span>
                                            </div>
                                          )}
                                          
                                          {(asig.fecha_entrega || asig.fecha_fin_estimada) && (
                                            <div>
                                              <span className="text-slate-500">Fecha de entrega estimada:</span>
                                              <span className="ml-1 font-medium text-slate-700">
                                                {new Date((asig.fecha_entrega || asig.fecha_fin_estimada).split('T')[0] + 'T00:00:00').toLocaleDateString('es-CO')}
                                              </span>
                                            </div>
                                          )}
                                        </div>
                                        ...
                                        <div className="flex-shrink-0">
                                          <span className="text-slate-500 text-xs">Valor asignado:</span>
                                          <span className="ml-1 font-bold text-slate-800">
                                            {new Intl.NumberFormat('es-CO',{style:'currency',currency:'COP',maximumFractionDigits:0}).format(Number(asig.valor_asignado)||0)}
                                          </span>
                                        </div>
                                      </div>
                                      ...
                                      {asig.observaciones && (
                                        <div className="mt-2 pt-2 border-t border-slate-200">
                                          ...
                                          <span className="text-xs text-slate-700 italic">{asig.observaciones}</span>
                                        </div>
                                      )}
                                    </div>
                                  );
      ...
      <div className="mt-6 bg-white rounded-lg p-4 shadow-sm border border-gray-200">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="text-sm text-gray-600 font-medium">
            Mostrando <span className="font-semibold text-gray-800">{costos.length > 0 ? ((page - 1) * pageSize + 1) : 0}</span> - <span className="font-semibold text-gray-800">{Math.min(page * pageSize, total)}</span> de <span className="font-semibold text-gray-800">{total}</span> costos
          </div>
